...
/datum/antagonist/prebel/can_be_owned(datum/mind/new_owner)
	. = ..()
	if(.)
		if(new_owner.assigned_role in GLOB.grove_positions)
			return FALSE
...
/datum/antagonist/prebel/head/on_gain()
	...
	owner.current?.set_patron(/datum/patron/inhumen/thief) //head rebels always love matthios, of course
...
/datum/antagonist/prebel/proc/can_be_converted(mob/living/candidate)
	...
	if(!can_be_owned(candidate.mind))
		return FALSE
	if(candidate.mind.assigned_role in GLOB.grove_positions)
		return FALSE
	if(candidate.mind.assigned_role in GLOB.divine_temple_positions)
		return FALSE
